  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await supabaseAdmin
      ...
      .insert({
        ...
        spreadsheet_id,
        sheet_name: data.csv_url,
        range_a1: "csv",
        column_map: {
          data: "data",
          vendedor: "vendedor",
          valor_liquido: "valor_liquido",
          clientes_liquido: "clientes_liquido",
        },
        ativo: data.ativo,
      })
      .select("*")
...
export const puxarVendasDoSheet = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    // 🔒 Segurança: sincronização manual só admin/gerente. Antes era aberto a qualquer
    // usuário autenticado, o que permitia disparar fetches externos e consumir quota.
    await ensureGestor(context.supabase, context.userId);
    ...
    try {
      ...
      await supabaseAdmin
        .from("sheet_sync_log")
        .insert({ direcao: "pull", linhas: 0, erro: e?.message ?? String(e) });
    ...
    const iD = idx("data"),
      iV = idx("vendedor"),
      iL = idx("valor_liquido"),
      iQ = idx("clientes_liquido");
    if (iD < 0 || iV < 0 || iL < 0)
      return {
        rows: [],
        linhas: 0,
        erro: `Colunas obrigatórias não encontradas em: ${headers.join(", ")}`,
      };
    const num = (s: string) => {
      const n = Number(
        String(s)
    ...
    const rows = table
      ...
      .map((r) => ({
        data: iso(r[iD] ?? ""),
        vendedor: (r[iV] ?? "").trim(),
        valor_liquido: num(r[iL] ?? "0"),
        clientes_liquido: iQ >= 0 ? Math.trunc(num(r[iQ] ?? "0")) : 0,
      }))
