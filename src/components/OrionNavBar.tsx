...
import {
  ...
  Menu, X, Sun, Moon, Heart, HelpCircle,
} from "lucide-react";
...
interface Props { paginaAtual: Pagina; onNavegar: (p: Pagina) => void; }
  ...
  return (
    <>
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-3 z-40 flex justify-center px-3 sm:bottom-6">
        <motion.nav
          initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ type: "spring", stiffness: 260, damping: 22 }}
        ...
        >
          {esquerda.map((it) => <BotaoNav key={it.id} item={it} ativo={paginaAtual === it.id} onClick={() => onNavegar(it.id)} />)}
          ...
          >
            ...
            <span className="pointer-events-none absolute -bottom-6 whitespace-nowrap text-[10px] font-semibold uppercase tracking-wider text-blue-300">{central.label}</span>
          </button>

          {direita.map((it) => <BotaoNav key={it.id} item={it} ativo={paginaAtual === it.id} onClick={() => onNavegar(it.id)} />)}
            ...
            >
              <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 text-xs font-bold text-white">{usuario.iniciais}</div>
                  <div><p className="text-sm font-semibold text-white">{usuario.nome}</p><p className="text-[11px] text-slate-500">{usuario.email}</p></div>
                </div>
                <button aria-label="Fechar" onClick={() => setDrawer(false)} className="rounded-lg p-1.5 text-slate-400 hover:bg-white/10 hover:text-white"><X className="h-5 w-5" /></button>
              </div>

              <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                {permitidos.concat(restantes).filter((v, i, a) => a.findIndex((x) => x.id === v.id) === i).map((it) => {
                  const ativo = paginaAtual === it.id;
                  return (
                    <button
                      key={it.id}
                      onClick={() => { onNavegar(it.id); setDrawer(false); }}
                      className={`flex flex-col items-center gap-1.5 rounded-2xl border p-3 text-center transition ${
                        ativo ? "border-blue-500/60 bg-blue-500/10 text-blue-300" : "border-white/5 bg-white/5 text-slate-300 hover:border-white/20 hover:bg-white/10 hover:text-white"
                      }`}
                    >
              ...
              <div className="mt-4 flex items-center justify-between gap-2 border-t border-white/10 pt-4">
                <button onClick={alternar} className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-slate-300 hover:bg-white/10 hover:text-white">
                  {tema === "claro" ? <Moon className="h-4 w-4" /> : <Sun className="h-4 w-4" />}
                  {tema === "claro" ? "Modo escuro" : "Modo claro"}
                </button>
                <button onClick={() => { void logout(); }} className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs font-semibold text-red-300 hover:bg-red-500/20">
...
function BotaoNav({ item, ativo, onClick }: { item: Item; ativo: boolean; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <span className="mt-0.5 hidden text-[9px] font-semibold uppercase tracking-wider sm:block">{label}</span>
    </button>
  );
}
