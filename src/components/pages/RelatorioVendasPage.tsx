...
export default function RelatorioVendasPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const { data } = await (supabase as any).from("metas_individuais").select("*").eq("usuario_id", usuario.id).order("periodo, categoria");
        setMetas(data || []);
  ...
  if (metas.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-12 text-center">
        ...
        <p className="font-medium text-slate-700">Nenhuma venda registrada</p>
        <p className="mt-1 text-sm text-slate-500">Suas vendas aparecerão aqui quando o gestor cadastrar seus dados.</p>
      </div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <h2 className="text-xl font-bold text-slate-800">Minhas Vendas</h2>
