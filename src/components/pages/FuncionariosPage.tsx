            ...
            return (
              <motion.div key={f.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div className="mt-3 flex gap-1.5">
                  ...
                  <button onClick={() => toggleAtivo(f)} className={cn("rounded-lg p-1.5 text-xs", f.ativo ? "text-slate-400 hover:bg-slate-100" : "text-emerald-500 hover:bg-emerald-50")} title={f.ativo ? "Desativar" : "Ativar"}>{f.ativo ? "⚪" : "🟢"}</button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        <div className="mb-4 flex items-center justify-between"><h3 className="text-lg font-semibold">Editar Funcionário</h3><button onClick={onClose}><X className="h-5 w-5 text-slate-400" /></button></div>
            ...
            <div className="space-y-2">
              {metas.map(m => (
                <div key={m.id} className="flex items-center justify-between rounded-lg border border-slate-200 p-3">
                  <div>
                    <p className="text-sm font-semibold capitalize">{m.categoria.replace(/_/g, " ")} <span className="text-xs text-slate-400">({m.periodo})</span></p>
