...
/**
 * Notifica o documento de que estamos em modo de impressão / exportação PDF.
 * Usado pelo CSS @media print e por um evento customizado para ocultar/mostrar
 * elementos fora deste componente (navbar, FAB, topbar, etc.).
 */
function exportarPDF() {
  if (typeof window === "undefined") return;
  // Sinaliza para qualquer ouvinte global que a impressão vai começar.
  // O OrionApp / Navbar / FAB poderiam reagir, mas aqui a estratégia é
  // simplesmente adicionar uma classe no <body> e removê-la após print.
  document.body.classList.add("orion-printing");
  // Dispara o diálogo nativo de impressão do navegador.
  // O usuário pode escolher "Salvar como PDF" como destino da impressora.
  window.print();
  // Limpa a flag logo após o diálogo fechar (best-effort).
  setTimeout(() => {
    document.body.classList.remove("orion-printing");
  }, 1000);
}
...
export default function ReportsView() {
    ...
    return (
      <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-12 text-center">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Seus relatórios aparecerão aqui quando houver dados.
        </p>
      </div>
    );
  }
  ...
  return (
    <>
      {/* CSS de impressão / exportação PDF — só surte efeito em window.print() */}
      <style>{`
        @media print {
          /* Oculta tudo que não pertence ao relatório */
          body > * { visibility: hidden !important; }
          /* Mas mostra o container raiz do OrionApp (para que o relatório fique visível) */
          .orion-report-printable, .orion-report-printable * { visibility: visible !important; }
          /* Posiciona o relatório no topo da página */
          .orion-report-printable {
          ...
          /* Oculta explicitamente elementos conhecidos (extra safety) */
          .orion-no-print, .orion-no-print * { display: none !important; }
          /* Remove sombras e bordas decorativas para impressão limpa */
          .orion-report-printable .rounded-xl,
          .orion-report-printable .rounded-2xl {
            box-shadow: none !important;
            break-inside: avoid;
          }
          /* Força cores reais (barras coloridas) mesmo em B&W preview */
          * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
          /* Quebra de página entre seções grandes, se necessário */
          .orion-print-page-break { break-before: page; }
          /* Remove dark mode para impressão */
          .dark .orion-report-printable { background: #ffffff !important; color: #0f172a !important; }
        }
      `}</style>
      ...
      <motion.div
        ...
        className="orion-report-printable space-y-6"
      >
        {/* Header da página com título + botão de exportar */}
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-bold text-slate-800">Meus Relatórios</h2>
            <p className="mt-0.5 text-xs text-slate-500">
              Gerado por <strong>{nomeUsuario}</strong> · {dataGerado}
            </p>
          </div>
          <button
            onClick={exportarPDF}
            className="orion-no-print inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-300"
            type="button"
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          <KpiCard
            label="Meta Total"
            value={brlMoeda(totalMeta)}
            Icon={Target}
            cor="text-blue-600"
          />
          <KpiCard
            label="Realizado"
            value={brlMoeda(totalRealizado)}
            Icon={DollarSign}
            cor="text-emerald-600"
          />
          <KpiCard
            label="Projeção"
            value={brlMoeda(totalProjecao)}
            Icon={TrendingUp}
            cor="text-indigo-600"
          />
          <KpiCard
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900">
          <h3 className="mb-4 text-sm font-bold uppercase tracking-wider text-slate-500">
            📊 Meta vs Realizado vs Projeção por Categoria
          </h3>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900">
            <h3 className="border-b border-slate-100 p-4 text-sm font-bold uppercase tracking-wider text-slate-500 dark:border-white/10">
              📋 Detalhes
            </h3>
                    ...
                    return (
                      <tr key={m.id} className="border-b border-slate-50 dark:border-white/5">
                        ...
                        <td className="px-4 py-2 text-right">
                          <span
                            className={`font-bold ${pctVal >= 100 ? "text-emerald-600" : pctVal >= 50 ? "text-amber-600" : "text-red-600"}`}
                          >
                            {pct(pctVal)}
                          </span>
                        </td>
    ...
    <div className="relative">
      <svg width="160" height="160" viewBox="0 0 160 160">
        ...
        <text
          ...
          y="75"
          textAnchor="middle"
          className="fill-slate-800 text-2xl font-bold dark:fill-white"
        >
          {pct.toFixed(0)}%
        </text>
